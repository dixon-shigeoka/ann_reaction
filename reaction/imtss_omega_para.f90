subroutine imtss_omega_para(n,delt,dtmp,dprs,aYi,omegai,totaldens)
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
!
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  integer, intent(in) :: n
  real(8), intent(in)  :: delt(n), dtmp(n), dprs(n), aYi(n,9)
  real(8), intent(out) :: omegai(n,8), totaldens
  ...
  real(8) :: atime, atmp, arho, aflag, art, atmpr, atmpl, aptr, adnsr, aprs
...
!---------------------------------------------------
  ...
  write(6,*) "before Loop Yi", aYi
  write(6,*) "before Loop prs", dprs
  write(6,*) "before Loop tmp", dtmp
  write(6,*) "before Loop delt", delt

!$OMP PARALLEL

  !$OMP DO PRIVATE (ii,m,i,atmp,atime,arho,aflag,art,ae01,ae02h,ae02l,ae03h)&
  !$OMP& PRIVATE (ae03l,ae04h,ae04l,ae05h,ae05l,ae06a,ae06b,ae07,ae08a,ae08b) &
  !$OMP& PRIVATE (ae09,ae10,ae11,ae12a,ae12b,ae13,ae14,ae15,ae16,ae17,ae18)   &
  !$OMP& PRIVATE (ae19,ae20,ae21,ae22,ae23,ae24,ae25,akf01,akf02h,akf02l)     &
  !$OMP& PRIVATE (akf03h,akf03l,akf04h,akf04l,akf05h,akf05l,akf06a,akf06b)    &
  !$OMP& PRIVATE (akf07,akf08a,akf08b,akf09,akf10,akf11,akf12a,akf12b,akf13)  &
  !$OMP& PRIVATE (akf14,akf15,akf16,akf17,akf18,akf19,akf20,akf21,akf22,akf23)&
  !$OMP& PRIVATE (akf24,akf25,ah2,ao2,ah,ao,aoh,ah2o,aho2,ah2o2,an2,am04,am05)&
  !$OMP& PRIVATE (am09,am21,am23,am24,am25,atrp,atrpl,atra,atrb)              &
  !$OMP& PRIVATE (atrf,akf02,akf03,akf04,akf05,akf06,akf08,akf12,itm,as1,as2) &
  !$OMP& PRIVATE (as3,as4,as5,as6,as7,as8,acon,ah1,ah3,ah4,ah5,ah6,ah7)       &
  !$OMP& PRIVATE (ah8,apt,ag,ahh01,ahh02,ahh03,ahh04,ahh05,ahh06,ahh07,ahh08) &
  !$OMP& PRIVATE (ahh09,ahh10,ahh11,ahh12,ahh13,ahh14,ahh15,ahh16,ahh17,ahh18)&
  !$OMP& PRIVATE (ahh19,ahh20,ahh21,ahh22,ahh23,ahh24,ahh25,akb01,akb02,akb03)&
  !$OMP& PRIVATE (akb04,akb05,akb06,akb07,akb08,akb09,akb10,akb11,akb12,akb13)&
  !$OMP& PRIVATE (akb14,akb15,akb16,akb17,akb18,akb19,akb20,akb21,akb22,akb23)&
  !$OMP& PRIVATE (akb24,akb25,akfc01,akfc02,akfc03,akfc04,akfc05,akfc06)      &
  !$OMP& PRIVATE (akfc07,akfc08,akfc09,akfc10,akfc11,akfc12,akfc13,akfc14)    &
  !$OMP& PRIVATE (akfc15,akfc16,akfc17,akfc18,akfc19,akfc20,akfc21,akfc22)    &
  !$OMP& PRIVATE (akfc23,akfc24,akfc25,akbc01,akbc02,akbc03,akbc04,akbc05)    &
  !$OMP& PRIVATE (akbc06,akbc07,akbc08,akbc09,akbc10,akbc11,akbc12,akbc13)    &
  !$OMP& PRIVATE (akbc14,akbc15,akbc16,akbc17,akbc18,akbc19,akbc20,akbc21)    &
  !$OMP& PRIVATE (akbc22,akbc23,akbc24,akbc25,ar01,ar02,ar03,ar04,ar05,ar06)  &
  !$OMP& PRIVATE (ar07,ar08,ar09,ar10,ar11,ar12,ar13,ar14,ar15,ar16,ar17,ar18)&
  !$OMP& PRIVATE (ar19,ar20,ar21,ar22,ar23,ar24,ar25,awh2,awo2,awh,awo,awoh)  &
  !$OMP& PRIVATE (awh2o,awho2,awh2o2,ath2,ato2,ath,ato,atoh,ath2o,atho2)      &
  !$OMP& PRIVATE (ath2o2,afg,adlt1,adlt2,adlt,atmx,asgm,aaa,adns,afm,ab)      &
  !$OMP& PRIVATE (abb,ajcb,aeng,aflgm1,aflgm2,aflgm3,aflgm4,aflgm5,aflgm6)    &
  !$OMP& PRIVATE (aflgm7,aflgm8,atmpr,atmpl,aptr,at,afmt,afdd,afd,af,addd,adt)&
  !$OMP& PRIVATE (ar,au,av,awk1,awk2,awk3,adnsr,aprs)                         &
  ...
  do ii = 1, n
    ...
    do i = 1, lsp
      call calc_phti(i,itm,atmp,phti,dmlr,dplh)
      ahti = phti
      ah   = ah  + adns(i)*ahti/arho
    end do
    aeng = dprs(ii) - ah*arho
...
!    revision
      ...
      adns(1:lsp-1) = max(0.d0, adns(1:lsp-1))
...
! -----------------------------------------
    ...
    aaa = abs(awh2)-aber
    abb = abs(awh2)/(adns(1)-awh2+aeps) - arer
    aflgm1 = a05+ sign(a05,aaa*abb*min(aaa,abb))
...
!-----------------------------------------------------------------------
  ...
  end subroutine imtss_omega_para
