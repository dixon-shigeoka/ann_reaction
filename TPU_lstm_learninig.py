...
from tensorflow.keras.layers import Dense,BatchNormalization,Activation, RepeatVector
...
for i in range(state_num) :
    ...
    ### make callbacks
    ### add for TensorBoard
    tb_cb = tensorflow.keras.callbacks.TensorBoard(log_dir=abspath_tflog,
                                                   #histogram_freq=1,
                                                   write_grads=True,
                                                   write_images=True
    )
    ...
    #                                        save_best_only=True
    #)
    es_cb = tensorflow.keras.callbacks.EarlyStopping(monitor='val_loss', patience=100, verbose=1,mode='min')
    #cbks = [tb_cb,cp_cb]
    cbks = [tb_cb]
    ###
