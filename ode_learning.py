...
##自作cost functionの実装
def net_ode (y_pred, train_x) :
    t = np.random.rand(1) * 1E-7
    dtmp = c.c_double(train_x(:,1))     #ctypes形式でラップ
    dprs = c.c_double(train_x(:,1))
    delt = c.c_double(delt_mem)
    #byrefでポインタにして渡す，mtsの実行
    fn.imtss_(c.byref(dtmp),c.byref(dprs),aYi,c.byref(delt),total_dens,omega)
    dtmp = dtmp.value
    dprs = dprs.value
    delt = delt.value
    total_dens = total_dens.value
    Yi_t = tf.gradients(Yi,t)[0]
    f = Yi_t - omega
